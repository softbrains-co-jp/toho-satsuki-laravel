...
class ConstOption extends Component
{
    ...
    public function mount(): void
    {
        ...
        if ($this->requestNumber) {
            $this->constOptionList = $this->constOptionQuery($this->requestNumber)->get();

            $selectedMerchantIds = $this->constOptionList
                ->pluck('okk_004')
    ...
    public function render()
    {
        return view('livewire.main.const-option', [
            'constOptionList' => $this->constOptionList,
        ]);
    }

    protected function constOptionQuery(string $requestNumber)
    {
        return VConstOptions::query()
            ->with(self::RELATIONS)
